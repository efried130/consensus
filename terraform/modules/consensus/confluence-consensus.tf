# Job Definition
resource "aws_batch_job_definition" "generate_batch_jd_consensus" {
  name = "${var.prefix}-consensus"
  type = "container"
  platform_capabilities = ["FARGATE"]
  propagate_tags = true
  tags = { "job_definition": "${var.prefix}-consensus" }

  container_properties = jsonencode({
    image = "${local.account_id}.dkr.ecr.us-west-2.amazonaws.com/${var.prefix}-consensus:${var.image_tag}"
...
resource "aws_cloudwatch_log_group" "cw_log_group" {
  name = "/aws/batch/job/${var.prefix}-consensus/"
}
